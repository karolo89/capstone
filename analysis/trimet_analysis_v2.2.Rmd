...
```{r}
df3 = df3 %>% mutate(max_line = case_when(
...
All `incident_type_code_list` rows have ASST, which means they are security incidents. This column doees not have value for us. 
```{r}
df3 %>% group_by(incident_type_code_list) %>% count()
```
