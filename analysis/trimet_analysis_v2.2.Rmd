...
blue_7pm = df3 %>% filter(max_line == "blue" & hour == 19) %>% count(location_id) %>% inner_join(df3 %>% select(location_id,location, lon, lat), by = "location_id") %>% distinct(location_id, n, lon, lat) %>% arrange(desc(n)) %>% head(n = 5) %>% distinct(location_id, n, lon, lat)
...
```
```{r}
blue_7p_tract = df3 %>% filter(max_line == "blue" & hour == 19) %>% count(tractce) %>% inner_join(df3 %>% select(tractce, geometry.y), by = "tractce") %>% distinct(tractce, n, geometry.y)
...
```{r}
df3 %>% filter(str_detect(comments, "[Ss]leep"))


df3 = df3 %>% mutate(sleeper = ifelse(str_detect(comments, "[Ss]leeper"), 1, 0))
df3 %>% filter(sleeper == 1) %>% group_by(location) %>% count() %>% arrange(desc(n))
```

Most sleepers found in gresham. 