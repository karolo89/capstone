...
*  2022 had significantly higher security incidents than the previous years, following a overall increasing trend.
*  Within that year, May through July has more activity, and April, Sep, and Oct had less.
...
ggplot(df_year_counts, aes(x = year, y = n, fill = color)) + 
  ...
  geom_label(data = df_year_counts %>% filter(year == 2022), aes(x = year, y = n, label = paste0(round(yoy, 2), "% +")), nudge_x = -1, fill = "white")
...
Our analysis of the months shows during the months of May through July, there were higher than expected security incidents. Let us examine where these events occurred. 
...
df_may_jul_2022 = inner_join(df_may_jul_2022, df3 %>% 
                  select(location_id, lon, lat, tractce, geometry.y) %>% 
...
```{r}
df_apr_sep_oct_2022 = df3 %>% filter(month %in% c("Apr", "Sep", "Oct")) %>% group_by(location, location_id) %>% summarize(n = n()) %>% arrange(desc(n))   
...
```
Yes, with more incidents in the Gresham area. 


Instead of view individual locations, let's examine how security incident are distributed by area.  
...
```
We see a band of high-incident areas across the city, east to west. Given that the locations with the most incidents are at transit centers across this parallel, it appears that security incidents occur around the MAX routes. 
...
max_blue = shape2 %>% filter(rte == 100 & dir == 0) %>% arrange(stop_seq)
...
incident_per_tract_2022 = df3 %>% 
  ...
  group_by(tractce) %>%
  count() %>% 
  left_join(df3 %>% select(tractce, namelsadco, geometry.y), by = "tractce") 
