...
```{r}
df <- read_csv('https://raw.githubusercontent.com/karolo89/capstone/main/data/trimet_security_2019_2023.csv')
summary(df)
...
We will treat nighttime as the time between sunset and sunrise.

### Questions: 

* What type of security incidents occurs most frequently as night? 

Adding seasonality category (night begins at different times according to season)
...
#ballpark nighttime as between the hours of 20:00 and 05:00 

df %>% group_by(night_time, season) %>% count()  %>% 
            ggplot(aes(x = season, y = n, fill = night_time)) + geom_col() + facet_grid(~ night_time) + 
            scale_discrete_manual(aesthetics = c("fill"), values = c("#FF7F7F", "#7F7FFF"))
...
What are the types of that occur most at `night_time`?
```{r}
df %>% group_by(night_time) %>% 
...
Where are the most incidents occuring at night? 
...
Gateway is a hot spot for all types of activity. 
Cleveland avenue is in NE Portland, it is worth investigating what lines run on this street. Cleveland is mentioned as a place where 'sleepers' are found. 


In 2023, have we seen a decrease in incidents (or rather, an increase in reports) at night given the increased presence of security personnel starting in March 2023? 

```{r}
df = df %>% mutate(y_m_d = date(df$incident_date)) # adding date only column 
...
```
Nighttime incidents were trending uppward in Jan - May 2022, and now they are trending downward Jan - May 2023. 
