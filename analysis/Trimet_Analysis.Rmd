...
shape2 %>% filter(str_detect(stop_name, "Elmonica")) # "Elmonica/SW 170th Ave MAX Station"
...
elmo = shape2 %>% filter(str_detect(stop_name, "Elmonica")) %>% filter(stop_seq == 1550)
...
routes = ggplot() + geom_sf(data = tri_county_tracts) + 
          geom_sf(data = shape2, aes(color = factor(rte)), alpha = .5, size = 1) + 
