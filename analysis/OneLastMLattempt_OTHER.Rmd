...
df3 %>%
  ...
  arrange(desc(count))
```
# I will focus on only the top 5 categories and and OTHER category for the remainig labeled groups (VANDAL, TRESPASS, ASLTCUST, FIGHT, ASLTEMPL)
# Creating my test and train dataset:
```{r}
#targets_of_interest <- c("VANDAL", "TRESPASS", "ASLTCUST", "FIGHT", "ASLTEMPL", "OTHER")
...
string_to_columns <- function(df, j = 500, stem=F){ 
  ...
  words <- words %>% 
    ...
    dplyr::select(-incident_subtype_code_list) %>%
...
#running my modified function on my full dataset
```{r}
gbm_cleaned <- string_to_columns(gbm_data, j = 1000, stem = T)
head(gbm_cleaned)
```
```{r}
targets_of_interest <- c("VANDAL", "TRESPASS", "ASLTCUST", "FIGHT", "ASLTEMPL", "OTHER")

gbm_cleaned_classed <- gbm_cleaned %>% 
  filter(target %in% targets_of_interest)
  
gbm_cleaned_unclassed <- gbm_cleaned %>% 
  filter(!target %in% targets_of_interest) 

gbm_cleaned_classed$target <- droplevels(gbm_cleaned_classed$target)

gbm_cleaned_classed
gbm_cleaned_unclassed
...
data_split <- initial_split(gbm_cleaned_classed %>% select(-incident_id,-comments), prop = 3/4)
trimet_train <- training(data_split)
trimet_test <- testing(data_split)

# set up the recipe
trimet_rec <-
  recipe(target ~ ., data = gbm_cleaned_classed %>% select(-incident_id,-comments)) %>%
...
# Obtain predictions
predictions <- trimet_fit %>% 
  predict(trimet_test) %>%
  bind_cols(trimet_test)

# Create confusion matrix
cm <- conf_mat(predictions, truth = target, estimate = .pred_class)
cm
...
```{r}
predictions_final <- trimet_fit %>% 
  predict(gbm_cleaned_unclassed) %>%
  bind_cols(gbm_cleaned_unclassed)

predictions_final %>% 
  group_by(.pred_class) %>% 
  summarise(.pred_class, count = n()) %>% 
  arrange(desc(count)) %>% 
  distinct()
```
# I will randomly sample 10 records from each class to judge accuracy
```{r}
sampled_df <- predictions_final %>%
  group_by(.pred_class) %>% # Replace 'class' with your column's actual name
  nest() %>%
  mutate(data = map(data, sample_n, size = 10)) %>% 
  unnest(data) %>% 
  select(.pred_class, incident_id,comments)

view(sampled_df)

asltempl_10samples = sampled_df %>% filter(.pred_class == "ASLTEMPL")
fight_10samples = sampled_df %>% filter(.pred_class == "FIGHT")
asltcust_10samples = sampled_df %>% filter(.pred_class == "ASLTCUST")
trespass_10samples = sampled_df %>% filter(.pred_class == "TRESPASS")
vandal_10samples = sampled_df %>% filter(.pred_class == "VANDAL")
other_10samples = sampled_df %>% filter(.pred_class == "OTHER")

view(fight_10samples)
```
