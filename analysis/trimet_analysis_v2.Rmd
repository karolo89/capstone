...
```
Tract # 008100 = Gateway Transit Center

```{r}
df2 %>% filter(tractce == '010001') %>% group_by(location) %>% count()
...
Tract # 010001 is far and away top dog - where is this ? *Gresham*
```{r}
df2 %>% filter(tractce == '010001') %>% group_by(location) %>% count()
```


plot(tri_county_tracts %>% filter(tractce == '010001')) # lol, cool fin 

incident_per_tract %>% 
  mutate(is_010001 = tractce == '010001') %>% 
  ggplot() +
  geom_sf(aes(geometry = geometry.y, fill = is_010001)) +
  coord_sf(xlim = c(-123.1, -122.4), ylim = c(45.3, 45.7)) + 
  labs(title = "Trimet Security Incident Hotspots, 2010-2023")
```
