...
df3 %>% filter(location_id == "10864") # Gateway Transit Center 
```
...
hotspots in 2021 2022 
```{r}
library(plotly)
incident_per_tract= df2 %>% filter(year == 2022) %>% group_by(tractce) %>%
  count() %>%
  ungroup %>%
  left_join(df2 %>% select(tractce,year, namelsadco, geometry.y), by = "tractce") 


map_plot = ggplot(incident_per_tract) +
  geom_sf(aes(geometry = geometry.y, fill = log(n))) + # logging n to condense the scale to show more variation 
  scale_fill_gradient(
      low = "white",
      high = "darkred",
